...
abstract class BaseAction extends Controller
{
    ...
    public function getRedirectUrl(float $amount, string $orderId): string
{
    $data = $this->createPayment($amount, $orderId);
    return config('jazzcash.' . env('JAZZCASH_ENVIRONMENT') . '.endpoint') . '?' . http_build_query($data);
}
