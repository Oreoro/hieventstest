...
{
    private const COLORS = ['black', 'red', 'black', 'yellow', 'blue', 'magenta', 'cyan', 'white', 'default'];
    ...
    public function progress(string $id, string $buffer, bool $error = false, string $prefix = 'OUT', string $errorPrefix = 'ERR'): string
    {
        ...
        if ($error) {
            ...
            if (!isset($this->started[$id]['out'])) {
                $message .= sprintf('%s<bg=black;fg=white> %s </> ', $this->getBorder($id), $prefix);
                $this->started[$id]['out'] = true;
            }

            $message .= str_replace("\n", sprintf("\n%s<bg=black;fg=white> %s </> ", $this->getBorder($id), $prefix), $buffer);
        }
    ...
    public function stop(string $id, string $message, bool $successful, string $prefix = 'RES'): string
    {
        ...
        if ($successful) {
            return sprintf("%s%s<bg=black;fg=white> %s </> <fg=black>%s</>\n", $trailingEOL, $this->getBorder($id), $prefix, $message);
        }
